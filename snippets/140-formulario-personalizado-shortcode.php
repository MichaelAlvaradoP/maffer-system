...
/**
 * ============================================================
 * SNIPPET 6 — FORMULARIO PERSONALIZADO MAFFER (SHORTCODE)
 * Nombre en WPCode: Formulario personalizado shortcode 6
 * Inserción: Run Everywhere
 * ============================================================
 * Uso: [maffer_formulario]
 *
...
function maffer_ajax_submit_form() {
    ...
    if ( empty( $nombre ) || strlen( $nombre ) < 2 ) {
        wp_send_json_error( [ 'campo' => 'nombre', 'mensaje' => 'El nombre es requerido.' ] );
    }
    if ( empty( $rut_raw ) ) {
        wp_send_json_error( [ 'campo' => 'rut', 'mensaje' => 'El RUT es requerido.' ] );
    }
    if ( empty( $menu_valor ) ) {
        wp_send_json_error( [ 'campo' => 'menu', 'mensaje' => 'Debes seleccionar un menú.' ] );
    }
    if ( ! $terminos ) {
        wp_send_json_error( [ 'campo' => 'terminos', 'mensaje' => 'Debes aceptar las condiciones.' ] );
    }
    ...
    if ( $estado !== 'abierto' ) {
        wp_send_json_error( [ 'mensaje' => 'El sistema está cerrado. No se aceptan registros en este momento.' ] );
    }

    // Normalizar y validar RUT
    $rut = strtoupper( preg_replace( '/\./', '', $rut_raw ) );
    if ( ! maffer_validar_rut_php( $rut ) ) {
        wp_send_json_error( [ 'campo' => 'rut', 'mensaje' => 'El RUT ingresado no es válido.' ] );
    }
    ...
    if ( $fecha_ciclo ) {
        $existe = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$tabla} WHERE rut = %s AND fecha >= %s",
            $rut, $fecha_ciclo
        ) );
    } else {
        $existe = $wpdb->get_var( $wpdb->prepare(
    ...
    if ( $result === false ) {
        wp_send_json_error( [ 'mensaje' => 'Error al guardar el registro. Intenta de nuevo.' ] );
    }
...
// Validación Módulo 11 en servidor
...
function maffer_render_formulario() {
    ...
    if ( function_exists( 'maffer_get_cierre_dt' ) ) {
        ...
        if ( $cl_dt ) {
            ...
            $dias_frm = array( 'Sunday'=>'domingo','Monday'=>'lunes','Tuesday'=>'martes',
    ...
    $menus_cena_raw = get_option( 'maffer_menu_cena', [] );
    $menus_json     = [ 'cena' => [] ];
            ...
            <div class="maffer-head">
                <div class="maffer-mark">
                    <div class="maffer-badge">
                        <img src="http://127.0.0.1:8081/wp-content/uploads/2026/04/LOGO-MAFFER-scaled.webp"
                             alt="Maffer"
