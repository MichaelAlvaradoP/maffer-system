...
/**
 * ============================================================
 * SNIPPET 7B — MAFFER: AJAX (EDITAR, ELIMINAR Y CREAR REGISTROS)
 * Nombre en WPCode: Maffer - Panel 7B Ajax
 * Tipo: PHP Snippet
 * Insercion: Run Everywhere
 * Activar DESPUÉS de snippet 7A.
 * ============================================================
 * CAMBIOS v2 (ciclo semanal / cena único):
 *  - turno eliminado de edición y creación; siempre se guarda 'cena'.
 *  - Verificación de duplicado usa ciclo: fecha >= fecha_ciclo en
 *    lugar de fecha = hoy AND turno = X.
 * ============================================================
 */

// ── EDITAR REGISTRO ──────────────────────────────────────────────────────────
add_action( 'wp_ajax_maffer_editar_registro', function () {
...
// ── ELIMINAR REGISTRO ────────────────────────────────────────────────────────
add_action( 'wp_ajax_maffer_eliminar_registro', function () {
    if ( ! check_ajax_referer( 'maffer_admin_nonce', 'nonce', false )
      || ( ! current_user_can( 'maffer_manage_menu' ) && ! current_user_can( 'manage_options' ) ) ) {
        wp_send_json_error( 'Sin permisos.' );
    }
...
add_action( 'wp_ajax_maffer_crear_registro', function () {
    if ( ! check_ajax_referer( 'maffer_admin_nonce', 'nonce', false )
      || ( ! current_user_can( 'maffer_manage_menu' ) && ! current_user_can( 'manage_options' ) ) ) {
        wp_send_json_error( 'Sin permisos.' );
    }
