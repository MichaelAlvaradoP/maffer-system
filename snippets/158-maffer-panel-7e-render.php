...
/**
 * ============================================================
 * SNIPPET 7E — MAFFER: RENDER DEL PANEL (HTML / CSS / JS)
 * Nombre en WPCode: Maffer - Panel 7E Render
 * Tipo: PHP Snippet
 * Inserción: Run Everywhere
 * Activar DESPUÉS de 7A, 7B, 7C (Excel), 7D (deprecado).
 * ============================================================
 * CAMBIOS v2 (ciclo semanal / cena único):
 *  - Panel Menús: solo Cena (eliminados tabs y panel Almuerzo).
 *  - Panel Config: datetime-local para apertura programada.
 *  - KPI 3: "Apertura programada" con datetime formateado.
 *  - Dashboard tabla: "Registros del Ciclo" (fecha >= fecha_ciclo).
 *    Columna "Turno" eliminada; columna "Fecha" agregada.
 *  - Modales crear/editar: sin selector de turno (siempre Cena).
 *  - JS: eliminado m6switchTab, MENUS_ALMUERZO, m6updateMenuSelect,
 *    lógica de horario en m6confirmToggle.
 * ============================================================
 */
...
function maffer_v6_render() {
    ...
    $t_exist = ( $wpdb->get_var( "SHOW TABLES LIKE '{$tabla}'" ) === $tabla );
    $registros = array();
    $total     = 0;
    $dist      = array();
    ...
    if ( $t_exist ) {
        $total     = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$tabla} WHERE fecha >= %s", $fecha_desde
        ) );
        $registros = $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$tabla} WHERE fecha >= %s ORDER BY fecha DESC, id DESC", $fecha_desde
        ), ARRAY_A );
        $dist_raw  = $wpdb->get_results( $wpdb->prepare(
            "SELECT menu_titulo, COUNT(*) cnt FROM {$tabla}
             WHERE fecha >= %s GROUP BY menu_titulo ORDER BY cnt DESC", $fecha_desde
        ), ARRAY_A );
        ...
        <div class="m6md m6mdsm">
            ...
            <div id="m6pwdok" style="display:none">
                ...
                <div class="m6mft" style="justify-content:center">
                    <a href="http://127.0.0.1:8081/admin-maffer/"
