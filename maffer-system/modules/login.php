<?php
/**
 * Maffer System — Module: Login Visual
 *
 * Custom login page branding and styling for Hotel Maffer.
 * Migrated from WPCode snippet 160 (Maffer - Login Visual).
 *
 * ## Function inventory
 *
 * | Function | Purpose |
 * |----------|---------|
 * | `maffer_login_logo_url()` | Filter login header URL to site homepage |
 * | `maffer_login_logo_text()` | Filter login header text to system name |
 * | `maffer_login_enqueue_fonts()` | Enqueue Google Fonts on login page |
 * | `maffer_login_head_css()` | Output custom login CSS with full Maffer branding |
 *
...
// ── Logo URL ─────────────────────────────────────────────────────
add_filter( 'login_headerurl', 'maffer_login_logo_url' );

if ( ! function_exists( 'maffer_login_logo_url' ) ) {
	/**
	 * Point login logo link to site homepage.
	 *
	 * @return string Home URL.
	 */
	function maffer_login_logo_url() {
		return home_url();
	}
}

// ── Logo Text ────────────────────────────────────────────────────
add_filter( 'login_headertext', 'maffer_login_logo_text' );

if ( ! function_exists( 'maffer_login_logo_text' ) ) {
	/**
	 * Custom login header title text.
	 *
	 * @return string System name.
	 */
	function maffer_login_logo_text() {
		return 'Sistema de Alimentacion Maffer';
	}
}

// ── Google Fonts ─────────────────────────────────────────────────
add_action( 'login_enqueue_scripts', 'maffer_login_enqueue_fonts' );

if ( ! function_exists( 'maffer_login_enqueue_fonts' ) ) {
	/**
	 * Enqueue Plus Jakarta Sans and Inter fonts for login styling.
	 *
	 * @return void
	 */
	function maffer_login_enqueue_fonts() {
		wp_enqueue_style(
			'maffer-login-fonts',
...
// ── Custom Login CSS ─────────────────────────────────────────────
add_action( 'login_head', 'maffer_login_head_css' );

if ( ! function_exists( 'maffer_login_head_css' ) ) {
	/**
	 * Output inline CSS for branded Maffer login page.
	 *
	 * Replaces the default WordPress login with a dark-themed,
	 * centered layout matching the Maffer brand identity.
	 *
	 * @return void
	 */
	function maffer_login_head_css() {
		// Logo URL: use content_url() so it works regardless of environment.
